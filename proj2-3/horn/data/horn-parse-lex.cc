...
#   endif
    ...
    private:
        ...
        void consolidate_values () {
            ...
            for (size_t i = rhs.size (); i > sym1; i -= 1) {
                ...
                if (!sym.missing ()) {
                    std::copy_backward (sym.v->begin () + sym._start, 
					sym.v->begin () + sym._end,
					yysemstack.begin () + _start);
                    std::copy_backward (sym.act->begin () + sym._start,
    ...
    public:
        /** As for assemble_values, but return the resulting values in
         *  a list, without disturbing the current stack contents. */
...
/* of interactive version */
    ...
    static std::vector<size_t> free_link;
    static std::vector<YYPARAM::value_type> tokens;
    static std::vector<yysembase::ACTION_TAG> act;
};
#endif
