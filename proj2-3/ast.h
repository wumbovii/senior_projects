...
public:
	...
	virtual void decorate(AST_Ptr root, Decl* decl_region, std::vector<Decl*>* global_decls, TargetEnum target_enum, Decl* builtin_decls);
	...
	virtual void rewriteMethods(AST_Ptr root);
	virtual void rewriteAttributes(AST_Ptr root);
	virtual Type_Ptr typeInference(AST_Ptr root, std::vector<Decl*> env);

	virtual void errorCall(Location_Type loc, std::string message);
...
public:
	 ...
	 *  variables. */
	enum TypeCategory {
		NONE = 0, LIST = 1, TUPLE = 2, INT = 4, STR = 8, DICT = 16,
		/** Denotes anything defined by "class" in the main program. */
		USER_CLASS = 32, FUNCTION = 64, UNRESTRICTED = (FUNCTION << 1) - 1
	};

	/** My category.  Returns 0 for a type variable. */
	virtual TypeCategory getCategory();

	virtual void setTypeCategory(TypeCategory category);
	...
	/** Setter and getter for state of type inference in a function type */
...
protected:
	...
	struct UnwindData {
		...
		UnwindData(Type_Ptr boundObject, int previousRestriction) :
			boundObject(boundObject), previousRestriction(previousRestriction) {
		}
...
static inline int _replace_and_incr(AST_Ptr& node, int& k, AST_Ptr& new_child) {
	int n = new_child == NULL ? 0 : new_child->is_list() ? new_child->arity() : 1;
