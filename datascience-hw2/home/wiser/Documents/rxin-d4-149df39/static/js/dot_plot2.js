d3.csv("preprocessed-data/statedon.csv", function handleCSV(csv) {
  ...
  vis.selectAll("circle")
     ...
     .attr("cy", y("CA"))
     .attr("r", r)
     .on("mouseover", function(d, i){d3.select("div#dot g.x").append("text").attr("id", "tip").attr("x", x(d)+5).attr("y", y("state")-5).attr("text-anchor", "start").attr("font-size", "8")
    	 		.text(function(s){ return data2[i].state;}); d3.select(this).attr("fill", "green")})
