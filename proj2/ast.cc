...
void AST::declare(AST_Ptr root, Decl* decl_region, TargetEnum target_enum) {
	...
	case CLASS_BLOCK: {
		...
		new_decl = makeClassDecl(decl_name, decl_set[0], Type::makeVar());
		new_decl->set_index(decl_index);

		//AST set DECL
		root->child(0)->setDecl(new_decl);
	...
	case DEF: {
		...
		//add Formals
		//If the def is an instance def (class container), then there must be
		//at least 1 parameter "self" of type class
		string formal_name;
		AST_Ptr formal_type;
		AST_Ptr formal;
		...
		for (size_t j = 0; j < decl_formals->arity(); j++) {
			formal = decl_formals->child(j);
			if(decl_region->get_dast_type() == CLASSD){
						formal = decl_formals->child(0);
						formal_name = formal->as_string();
						string self = "self";

						if(formal_name.compare(self) == 0){
							//the first param is self
							continue;

						} else {
							//the first param is missing self, throw error
							Error(root->lineNumber(), "Bogus Instance Definition: First parameter must be of \"self\"");
						}
...
void AST::decorate(AST_Ptr root, Decl* decl_region, AST_Ptr parent,
		int child_number) {
	...
	int index;
	Decl* member;
	vector<Decl*> decl_region_members;
	Decl* cur_node = getDecl();
	const char * cur_node_name_char = root->chars_dup();
	string cur_node_name = root->as_string();

	if (cur_node != NULL && syntax != ID) {

		//we are at a classblock/def/module node
		//change region
		decl_region = cur_node;
	}

	if (syntax == ID) {
		//we are at an ID node, time to decorate
		decl_region_members = decl_region->get_members();
		for (size_t i = 0; i < decl_region_members.size(); i++) {
			member = decl_region_members[i];

			if ((member->get_name()).compare(cur_node_name) == 0) {
				//we found a match, they must be the same and unique
				//establish the index
				index = member->get_index();
			}
		}
		...
		AST_Ptr new_child;
		char dec_node[256];
		string dec_node_string;
		std::sprintf(dec_node, "(id %d %s %d)", root->lineNumber(),
				cur_node_name_char, index);
		dec_node_string = dec_node;
		new_child = make_token(syntax, dec_node_string, false);

		//remove old node, insert new one;
		parent->replace(child_num, new_child);

	}

	for (int i = 0; i < num_children; i++) {
		decorate(root->child(i), decl_region, root, i);
	}
