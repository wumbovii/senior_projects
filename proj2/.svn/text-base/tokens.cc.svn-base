...
void AST_Token::_print(ostream& out, int indent) {
	out << "(<Token>)";
}
...
protected:
	...
	Int_Token* post_make() {
		...
		if (errno != 0 || value > (1L << 30)) {
			Error(loc(), "literal value out of range: %s", text.c_str());
			value = 0;
		}
		return this;
	}
...
protected:
	void _print(ostream& out, int indent) {
		...
		Decl* declaration = getDecl();
		if (declaration != NULL) {
			out << " " << declaration->get_index();
		}
		out << ")";
	}
